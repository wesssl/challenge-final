...
Obter Informações de Sessões
    ...
    ${response}    GET On Session    CinemaApp    /sessions    expected_status=any
    Validar Sucesso na Requisição    ${response}
    Validar se Response é JSON    ${response}

Obter Informação de Sessão por ID
    [Arguments]    ${id}
    Create Session    CinemaApp    ${BASEURL}
    ${response}    GET On Session    CinemaApp    /sessions/${id}    expected_status=any
    Validar Sucesso na Requisição    ${response}
    Validar se Response é JSON    ${response}
